...
import { addDriverWalletAction } from "../../actions/AdminActions";
const danger = { mr: "5px", border: "solid 1px red" };

const DriverAddWalletForm = ({ 
    handleClose,
    id
}) => {
    ...
    const handleSubmit =() => {
        ...
        if(amount && discription){
            dispatch(addDriverWalletAction(id, amount, discription))
            handleClose()
        }
    }
...
export default DriverAddWalletForm;
